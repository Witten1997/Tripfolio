# Tripfolio P0 数据库表结构设计 v0.6

日期：2026-09-11；v0.2 修订于 2026-09-12；v0.3 修订于 2026-09-12  
状态：详细设计稿，供用户审阅；字段、约束和索引尚未转化为迁移执行。  
v0.6 修订于 2026-09-19：新增第 25 张表 trip_members 与第 26 张表 ledger_entry_splits；ledger_entries 增加 split_count、personal_amount、payer_member_id、split_mode；sync_changes 的 entity_type 枚举增加 trip_member。迁移 00014。  
...
| trip_members | 旅行成员与分摊百分比 |
| ledger_entry_splits | 账目按成员的分摊份额 |
| reservations | 预订 |
| documents | 独立资料与预订附件 |
...
| split_count | integer | NOT NULL DEFAULT 1；CHECK 1–9999 | 参与分摊的成员数，由服务端按 splits 派生 |
| personal_amount | numeric(18,4) | NOT NULL DEFAULT 0；CHECK >= 0 | 「我」（is_self 成员）的份额；不参与时为 0；统计的个人口径 |
| payer_member_id | uuid | NOT NULL | 付款人（退款为收款人），复合外键 (account_id, trip_id, payer_member_id) → trip_members |
| split_mode | varchar(8) | NOT NULL DEFAULT 'even'；CHECK even/ratio | 分摊模式 |

键与关联：

- T 组键；原支出使用 (account_id, trip_id, refunded_entry_id) 三列外键；分类使用 (account_id, category_id) 两列外键；付款人使用 (account_id, trip_id, payer_member_id) 三列外键。
...
- 分摊份额存 ledger_entry_splits，由服务端按 split_mode 计算并整体替换；修改成员百分比不重算历史份额。
...
### 25. trip_members：旅行成员与分摊百分比

| 字段 | PostgreSQL 类型 | 空值 / 默认 / 约束 | 说明 |
| --- | --- | --- | --- |
| id | uuid | 主键 | 客户端生成 |
| account_id | uuid | NOT NULL | 外键 accounts.id |
| version | bigint | NOT NULL DEFAULT 1；CHECK > 0 | 资源版本 |
| created_at / updated_at / deleted_at | timestamptz | 同 T 组 | 软删除 |
| trip_id | uuid | NOT NULL | 复合外键 (account_id, trip_id) → trips(account_id, id) |
| name | varchar(30) | NOT NULL；CHECK 1–30 且去空格非空 | 成员名称 |
| share_percent | numeric(5,2) | NOT NULL；CHECK 0–100 | 分摊百分比；全体有效成员之和由服务端保证为 100 |
| sort_order | integer | NOT NULL DEFAULT 0 | 显示顺序 |
| is_self | boolean | NOT NULL DEFAULT false | 「我」；每趟旅行恰有一个有效 is_self，不可删除 |

键与关联：UNIQUE (account_id, trip_id, id)；部分唯一索引 (account_id, trip_id, lower(btrim(name))) WHERE deleted_at IS NULL；部分唯一索引 (account_id, trip_id) WHERE is_self AND deleted_at IS NULL；索引 (account_id, trip_id, sort_order, id) WHERE deleted_at IS NULL。

规则：进入同步流，entity_type=trip_member。创建旅行时同事务插入「我」；迁移 00014 为存量旅行补齐并把存量账目的付款人回填为「我」。删除前检查 ledger_entries.payer_member_id 与 ledger_entry_splits 的有效引用。

### 26. ledger_entry_splits：账目按成员的分摊份额

| 字段 | PostgreSQL 类型 | 空值 / 默认 / 约束 | 说明 |
| --- | --- | --- | --- |
| account_id / trip_id / ledger_entry_id | uuid | NOT NULL | 复合外键 → ledger_entries(account_id, trip_id, id) |
| member_id | uuid | NOT NULL | 复合外键 (account_id, trip_id, member_id) → trip_members |
| amount | numeric(18,4) | NOT NULL；CHECK >= 0 | 该成员份额，按币种最小单位 |
| sort_order | integer | NOT NULL DEFAULT 0 | 参与人顺序，余数分配依据 |

键与关联：PRIMARY KEY (account_id, trip_id, ledger_entry_id, member_id)；索引 (account_id, trip_id, member_id) 供删除成员前的引用检查与结算聚合。不进入同步流，随 ledger_entry 的 splits 字段一起同步。
