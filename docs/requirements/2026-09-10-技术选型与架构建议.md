...
| 主数据库 | PostgreSQL 18（开发与 CI），生产 ≥ 13 且优先托管 | 账号、旅行、预算、账单分类、账目、预订、资料、照片、文件元数据、同步记录 |
...
本次核对时，Go 1.27 与 1.26 属于官方受支持分支；新项目优先选择依赖验证通过的 Go 1.27.x。Go 的支持策略与 Node.js LTS 命名不同，需要跟进受支持分支的更新。PostgreSQL 18 处于官方支持期；若托管数据库只提供受支持的 PostgreSQL 17，可在一致性测试后采用 17。**自托管部署的支持下限为 PostgreSQL 13**（2026-09-16 在 NAS 上实测：迁移与查询不使用 15+ 专有语法，头像外键的删除语义由 BEFORE DELETE 触发器实现），更老的 12 及以下未验证；新部署仍建议 15+。前端构建可使用 Node.js 24 LTS 与 pnpm，后端部署 Go 编译产物。
