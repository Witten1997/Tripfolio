...
import { ApiError, resetPassword } from '@/shared/api/auth'
import {
  isEmail,
  isSixDigits,
  isValidPassword,
...
const challenge = useEmailChallenge('reset_password')
    ...
    <ElCard class="auth-card">
      ...
      <template v-if="done">
        <ElAlert
          type="success"
          title="密码已重置"
          description="所有设备已退出登录，请用新密码重新登录。"
          :closable="false"
          show-icon
          class="auth-alert"
        />
        <ElButton type="primary" class="auth-submit" @click="router.replace({ name: 'login' })">
          去登录
        </ElButton>
      </template>
      <template v-else>
